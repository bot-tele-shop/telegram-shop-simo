...
def test_start_matches_complete_reference_layout_without_fake_wallet(harness):
...
def test_start_installs_persistent_quick_access_keyboard(harness):
    feed(harness, message=message("/start"))
    quick = [
        call
        for call in harness.session.calls
        if isinstance(call, SendMessage) and getattr(call.reply_markup, "keyboard", None)
    ]
    assert len(quick) == 1
    assert [[button.text for button in row] for row in quick[0].reply_markup.keyboard] == [
        ["🛍 Products", "🛟 Support"],
        ["⭐ Payments"],
        ["🔗 API"],
    ]
...
def test_runtime_presentation_copies_are_identical():
    ...
    assert (root / "shop/storefront.py").read_bytes() == (
        root / "worker/src/storefront.py"
    ).read_bytes()
